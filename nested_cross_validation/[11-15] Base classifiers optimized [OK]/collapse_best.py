...
def main():
    ...
    for clf in classifiers:
        ...
        for experiment_folder in experiment_folders:
            dataset_name = [x for x in os.listdir(os.path.join('.', clf, experiment_folder)) if os.path.isdir(os.path.join('.', clf, experiment_folder, x))][0]
            ...
            parameter_files = [x for x in os.listdir(os.path.join('.', clf, experiment_folder, dataset_name)) if os.path.isfile(os.path.join('.', clf, experiment_folder, dataset_name, x))]

            for parameter_file in parameter_files:
                n_fold = int(parameter_file[parameter_file.index('fold-') + len('fold-'):parameter_file.index('_parameters')])
    ...
    for dataset_name in final.keys():
        final[dataset_name] = dict(((x, '-Aggregator MajorityVotingAggregator ') for x in range(min_fold, max_fold + 1)))
        for n_fold in range(min_fold, max_fold + 1):

            _on_hold = None
            for clf in ev[dataset_name].keys():
                if clf != 'DecisionTable':
                    final[dataset_name][n_fold] += '-%s %s' % (clf, ' '.join(ev[dataset_name][clf][n_fold].values()).strip() + ' ')
                else:
                    final[dataset_name][n_fold] += '-%s ' % clf

                    for param in ev[dataset_name][clf][n_fold].values():  # type: str
                        if '-S' in param:
                            search_alg_name = None
                            if 'BestFirst' in param:
                                search_alg_name = 'BestFirst'
                            elif 'GreedyStepwise' in param:
                                search_alg_name = 'GreedyStepwise'

                            final[dataset_name][n_fold] += '-S weka.attributeSelection.' + search_alg_name + ' '
                            _on_hold = '-%s %s' % (search_alg_name, param.split(search_alg_name)[1])
                        else:
                            final[dataset_name][n_fold] += param + ' '

            final[dataset_name][n_fold] += _on_hold
